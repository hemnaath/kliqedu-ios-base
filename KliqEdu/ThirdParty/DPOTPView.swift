...
fileprivate class OTPBackTextField: UITextField {
 ...
 //        return super.canPerformAction(action, withSender: sender)
     ...
     override func paste(_ sender: Any?) {
         guard let otp = UIPasteboard.general.string?
             .trimmingCharacters(in: .whitespacesAndNewlines) else { return }
  
         // Allow only numeric characters
         let numericSet = CharacterSet.decimalDigits
         guard CharacterSet(charactersIn: otp).isSubset(of: numericSet) else { return }
  
         // Limit to the number of fields available
         let maxLength = min(otp.count, dpOTPView.count)
         for i in 0..<maxLength {
             let char = otp[otp.index(otp.startIndex, offsetBy: i)]
             let tf = dpOTPView.arrTextFields[i]
             tf.text = String(char)
             dpOTPView.dpOTPViewDelegate?.dpOTPViewAddText(String(char), at: i)
         }
  
         // Focus next empty field
         if maxLength < dpOTPView.count {
             dpOTPView.arrTextFields[maxLength].becomeFirstResponder()
         } else {
             dpOTPView.arrTextFields.last?.resignFirstResponder()
         }
     }
